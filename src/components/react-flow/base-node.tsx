...
export function BaseNode({ className, status, ...props }: BaseNodeProps) {
    return (
        ...
        >
            {props.children}
            {status === "error" && (
                <XCircleIcon className="absolute right-0.5 bottom-0.5 text-red-700 stroke-3" />
            )}

            {status === "success" && (
                <CheckCircle2Icon className="absolute right-0.5 bottom-0.5 text-green-700 stroke-3" />
            )}

            {status === "loading" && (
                <Loader2Icon className="absolute -right-0.5 -bottom-0.5 text-blue-700 stroke-3" animate-spin />
            )}
        </div>
    );
}
