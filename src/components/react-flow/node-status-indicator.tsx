...
const StatusBorder = ({
    ...
    return (
        <>
            <div
                className={cn(
                    "absolute -top-[2px] -left-[2px] h-[calc(100%+4px)] w-[calc(100%+2=4px)] rounded-md border-3",
                    className,
                )}
            />
            {children}
...
export const NodeStatusIndicator = ({
    ...
    switch (status) {
        ...
        case "error":
            return <StatusBorder className={cn("border-red-700/50", children)}> {children}</ StatusBorder>;
        default:
            return <>{children}</>;
    }
};
